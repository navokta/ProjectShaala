    ...
    try {
      ...
      const res = await fetch(`/api/admin/bids/${id}`, {
        ...
        body: JSON.stringify({
          ...
          bidAmount: bid.bidAmount || bid.amount, // handle schema differences if any
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col font-inter">
        <div className="flex-1 max-w-7xl mx-auto w-full px-6 py-10">
          <div className="bg-white p-8 rounded-xl border border-gray-200 text-center">
            ...
            <Link href="/admin/bids" className="text-blue-600 hover:underline">
              Return to Bids
            </Link>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="max-w-5xl mx-auto w-full px-6 py-10 flex-1">
          <div className="mb-6 flex items-center justify-between">
            ...
            <Link href="/admin/bids" className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition">
              Back to Bids
            </Link>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                ...
                <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium border mb-4 ${
                  ...
                  bid.status === 'completed' ? 'bg-blue-50 text-blue-700 border-blue-200' :
              ...
              {bid.developer && (
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Email:</span> {bid.developer.email}</p>
                    {bid.developer.title && <p><span className="font-medium">Title:</span> {bid.developer.title}</p>}
                    {bid.developer.location && <p><span className="font-medium">Location:</span> {bid.developer.location}</p>}
                    <Link href={`/admin/users/${bid.developer._id}`} className="text-blue-600 hover:underline inline-block mt-2">
                      View Full Profile
                    </Link>
                  </div>
                </div>
              ...
              {bid.project && (
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                  ...
                  {bid.project.budget && (
                    <p className="text-sm"><span className="font-medium">Project Budget:</span> ₹{bid.project.budget.toLocaleString()}</p>
                  )}
                  {bid.project.status && (
                    <p className="text-sm mt-1"><span className="font-medium">Project Status:</span> {bid.project.status}</p>
                  )}
